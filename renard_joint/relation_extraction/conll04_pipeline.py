...
ner_model = pickle.load(open(os.path.join(os.environ.get("MODEL", "model"), "ner/conll04_nn_1024.model"), 'rb'))

print("Loading relation extraction model...")
re_model = model.BertForMre(len(relation_classes) + 1)
re_model.load_state_dict(torch.load(os.path.join(os.environ.get("MODEL", "model"), "re/conll04_100.model"),
                                    map_location=device))
...
def evaluate(group,
    ...
    results.to_csv(os.path.join(os.environ.get("MODEL", "model"), "re/conll04_evaluate_" + group + ".csv"))
    print(results)
